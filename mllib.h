...
class linear_regression{
    ...
    public:
        ...
        void sgd_train(numcpp<numcpp<double>> features, numcpp<double> labels, double lr = 0.01, long long int epochs = 100){
            ...
            for (long long int i = 0, i < epochs, i++){
                ...
                curr_loss = mse_loss(predicted, label);
                if (i > 0) cout << "Training Loss: " + curr_loss + " | " + "Epoch: " + i << endl;
                feature.elemwise_update([](double x){return mse_loss(predicted + x * h, label)});
